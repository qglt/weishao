...
namespace gloox
{
  ...
  void MessageSession::send( const std::string& message, const std::string& subject, const std::string& html, const StanzaExtensionList& sel )
  {
    if( !m_hadMessages )
    {
      m_thread = "gloox" + m_parent->getID();
    ...
    Message m( Message::Chat, m_target.bare(), message, subject, html, m_thread );
