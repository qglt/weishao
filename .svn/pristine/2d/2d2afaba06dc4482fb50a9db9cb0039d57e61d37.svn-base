...
//这个值间隔加图片大小
static const NSInteger kStarWidthAndHeight = 17;
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    //        此处添加代理方法，把星评的分数传到控制器，让控制器执行
    if (self.ratingDelegate && [self.ratingDelegate respondsToSelector:@selector(commentRating:)]) {
        [self.ratingDelegate commentRating:_rating];
    }
