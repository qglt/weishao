...
struct QRcodeImpl
{
	static const int casesensitive = 1;
	static const int eightbit = 1;
	static const int version = 7;
	static const int size = 3;
	static const int micro = 0;
	static const QRecLevel level = QR_ECLEVEL_H;
	static const QRencodeMode hint = QR_MODE_8;
	int margin;
	QRcodeImpl();
	int writeSVG(QRcode *qrcode, const char *outfile,std::string colorStr);
...
int QRcodeImpl::writeSVG( QRcode *qrcode, const char *outfile,std::string colorStr)
{
	std::ofstream ofs;
	unsigned char *row, *p;
	int x, y, yy, xx;
	...
	epius::epfilesystem::instance().open_stream(outfile, ofs, std::ios::binary);

	realwidth = (qrcode->width + margin * 2) * size;
	double fsize = size*100.0/realwidth;
	/* SVG file header */
	ofs<<"<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" width=\"100%\" height=\"100%\">\n\t<g>\n";
	/* data */
	p = qrcode->data;
	for(y=0; y<qrcode->width; y++) {
		...
		for(x=0; x<qrcode->width; x++) {
			if(*(row+x)&0x1) {
				xx = (x + margin) * size;
				double fxx = xx*100.0/realwidth;
				double fyy = yy*100.0/realwidth;
				ofs<<boost::format("\t\t<rect x=\"%.2f%%\" y=\"%.2f%%\" width=\"%.9f%%\" height=\"%.9f%%\" style=\"fill:%s\"/>\n") %fxx %fyy %fsize %fsize %colorStr.c_str();
			}
		}
	}
	ofs<<"\t</g>\n</svg>";
...
QRcode * QRcodeImpl::encode( const unsigned char *intext, int length )
{
	QRcode *code;
...
void QRcodeMgr::qrencode( const char *intext, int length, const char *outfile,std::string colorStr)
{
	QRcodeImpl impl;
	QRcode *qrcode;
	qrcode = impl.encode((const unsigned char*)intext, length);
	if(qrcode == NULL) {
		return;
	}
	impl.writeSVG(qrcode, outfile, colorStr);
	QRcode_free(qrcode);
}
