...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _text = @"";
        _font = [UIFont fontWithName:@"STHeitiSC-Thin" size:11.0f];
...
- (void)drawRect:(CGRect)rect
{
    ...
    [attString setTextAlignment:kCTNaturalTextAlignment lineBreakMode:kCTLineBreakByTruncatingMiddle];
    //设置字体
    ...
    float drawLineY = self.bounds.origin.y + self.bounds.size.height - self.font.ascender-1.0;
    BOOL drawFlag = YES;
...
goto check;
        ...
        for (int i = 0; i < CFArrayGetCount(runs); i++)
        {
            ...
            if (textRun)
            {
                ...
                //图片需要调小到宽和高为8*8
                CGRect runRect = CGRectMake(runPointX, runPointY, runWidth-5.2, runHeight-5.2);
