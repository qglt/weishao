...
namespace gloox
{
                               ...
                               public IqHandler
  {
	...
	public:
	// 登入时缓存所有message, 等界面加载完成后再处理
	boost::shared_ptr<boost::signal<void(void)> > process_msg_;
	virtual void process_hanlded_message() = 0;
    ...
    public:
       ...
       * Default: @b false.
       */
      void send( IQ& iq, IqHandler* ih, int context, bool del = false, bool needAddFrom =true, bool needAddNamespace =true );
	  
      /**
       * A convenience function that sends the given IQ stanza.
       * @param iq The IQ stanza to send.
       */
      void send( const IQ& iq, bool needAddFrom =true, bool needAddNamespace =true );
    ...
    protected:
	  ...
	  void bind_servertime(const Tag* tag);
	  tm get_servertime() const;
	  long long get_timetick() const;
      /**
    ...
    public:
	  ...
	  virtual void rosterFilledNoPresence() = 0;
      ...
      void notifyMessageHandlers(Tag* tag);
	  void donotifyMessageHandlers( boost::shared_ptr<Tag> shared_tag );
