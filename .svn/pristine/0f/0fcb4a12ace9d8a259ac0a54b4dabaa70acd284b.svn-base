...
namespace json
{
	...
	protected:
		...
		struct create_pair
		{
			...
			void operator() (Iterator /* szStart */, Iterator /* szEnd */ ) const
			{
                ...
                if(try_any_cast(m_stack.top(),strName))
                {
                    std::basic_string< typename Iterator::value_type > tmp = *strName;
                    m_stack.pop();
                    // 3: push a pair of both on the stack...
                    m_stack.push(variant(new boost::any(pair(tmp, var))));
                }
			}
		};
	...
	public:
		stack & m_stack;
		bool m_bIsObject;
		grammar(stack & stack) : m_stack(stack), m_bIsObject(true) { }
		void do_parse(std::basic_string< Char > str)
		{
			using namespace boost::spirit::classic;
			//first, try to parse as an object
			parse_info<typename std::basic_string< Char >::iterator> pi = boost::spirit::classic::parse(str.begin(), str.end(), *this, space_p | ch_p('#') >> *(anychar_p - eol_p) >> eol_p | str_p("//") >> *(anychar_p - eol_p) >> eol_p | str_p("/*") >> *(anychar_p - str_p("*/")) >> str_p("*/"));
			//second, try to parse as array
			if(m_stack.size()==0 && pi.stop !=str.end())
			{
				m_bIsObject = false;
				pi = boost::spirit::classic::parse(str.begin(), str.end(), *this, space_p | ch_p('#') >> *(anychar_p - eol_p) >> eol_p | str_p("//") >> *(anychar_p - eol_p) >> eol_p | str_p("/*") >> *(anychar_p - str_p("*/")) >> str_p("*/"));
			}
		}
		...
		public:
			boost::spirit::classic::rule< SCANNER > const & start() const 
			{ 
				if(m_self.m_bIsObject)
					return m_object; 
				else
					return m_array;
			}
