...
//       m_parent->removeStanzaExtension( ExtMUCOwner );
  ...
  void MUCRoom::send( const std::string& message, const std::string& html )
  {
    if( m_session && m_joined )
      m_session->send( message,html );
  }
  ...
  void MUCRoom::storeListNone( const MUCListItemList items )
  {
	  if( !m_parent || !m_joined )
		  return;

	  IQ iq( IQ::Set, m_nick.bareJID() );
	  iq.addExtension( new MUCAdmin( SetANone , items ) );
	  m_parent->send( iq, this, SetANone );
  }
  ...
  void MUCRoom::handleMessage( const Message& msg, MessageSession* /*session*/ )
  {
    ...
    else
    {
      ...
      else if( !msg.body().empty() || !msg.html().empty() )
      {
        std::string when;
        bool privMsg = false;
        bool history = false;
