...
#include "biz/biz_anan/biz_src/thread_id.h"
...
namespace gloox
{
  ...
  void RosterManager::handleIqID( const IQ& iq, int context )
  {
    if( iq.subtype() == IQ::Result ) // initial roster
    {
      ...
      if( context == RequestRoster )
      {
		  // 发送2次重复的presence
        if( m_parent )
          m_parent->rosterFilledNoPresence();
  ...
  void RosterManager::handlePresence( const Presence& presence )
  {
	...
	if(presence.findExtension(kExtUser_presence_filter_threadid))
	{
		boost::shared_ptr<gloox::Tag> tag(presence.findExtension(kExtUser_presence_filter_threadid)->tag());
		std::string id = tag->findCData("ext_threadid");

		if (biz::thread_id::instance().is_id_exist(id))
		{
			return;
		}
	}
  ...
  std::string RosterManager::add( const JID& jid, const std::string& name, const StringList& groups )
  {
    if( !jid )
      return EmptyString;

	std::string newID = m_parent->getID();

    IQ iq( IQ::Set, JID(), newID );
  ...
  void RosterManager::handleSubscription( const Subscription& s10n )
  {
    ...
    switch( s10n.subtype() )
    {
      ...
      {
		std::string status = s10n.status();
        m_rosterListener->handleItemUnsubscribed( s10n.from(), status);
        break;
      }
  ...
  void RosterManager::handlePrivateXML( const std::string& uid, const Tag* xml )
  {
    if( xml )
