...
namespace gloox
{
  ...
  {
	memset(&m_server_base_time, 0,sizeof(m_server_base_time));
    init();
  }
  ...
  {
	//ELOG("log_network")->debug("send:\t"+data);
  ...
  {
	//ELOG("log_network")->debug("receive:\t"+data);
    parse( data );
  }
  ...
  void ClientBase::handleConnect( const ConnectionBase* /*connection*/ )
  {
	  // 延时8秒处理离线消息
	  // 离线消息只触发一次提示音
	  // 群和讨论组的名字没有在会话协议中 需等待几秒钟获取后再保存聊天记录 并通知界面
	  process_msg_.reset(new boost::signal<void(void)>);

	  epius::time_mgr::instance().set_timer(8000, boost::bind(&ClientBase::process_hanlded_message, this));

	  //header();
	  notifyStreamEvent(StreamEventConnecting);
  }
...
#include <sys/time.h>
  ...
  void ClientBase::bind_servertime(const Tag* tag)
  {
#ifndef __APPLE__
#ifdef _WIN32
	  m_server_base_time.m_tick = GetTickCount();
#else
	  struct sysinfo info;
	  sysinfo(&info);
	  m_server_base_time.m_tick = info.uptime*1000;
#endif
#else
      m_server_base_time.m_tick = static_cast<long long>(uptime());
#endif

	  std::string timestamp_filter = "/success[@xmlns='urn:ietf:params:xml:ns:xmpp-sasl']";
	  if (const Tag *child_tag = tag->findTag(timestamp_filter)) {
		  std::string timestamp_string = child_tag->findAttribute("timestamp");
		  if (timestamp_string.empty()) {
			  memset(&m_server_base_time.m_time, 0,sizeof(m_server_base_time.m_time));
		  } else {
// 			  timestamp="2012-10-16 14:39:21.264";
			  m_server_base_time.m_time.tm_year = atoi(timestamp_string.substr(0,4).c_str());
			  m_server_base_time.m_time.tm_mon = atoi(timestamp_string.substr(5,2).c_str());
			  m_server_base_time.m_time.tm_mday = atoi(timestamp_string.substr(8,2).c_str());
			  m_server_base_time.m_time.tm_hour = atoi(timestamp_string.substr(11,2).c_str());
			  m_server_base_time.m_time.tm_min = atoi(timestamp_string.substr(14,2).c_str());
			  m_server_base_time.m_time.tm_sec = atoi(timestamp_string.substr(17,2).c_str());
		  }
	  } else {
		  memset(&m_server_base_time.m_time, 0,sizeof(m_server_base_time.m_time));
	  }
...
//         etc... see gssapi-sasl-draft.txt
  ...
  void ClientBase::send( IQ& iq, IqHandler* ih, int context, bool del, bool needAddFrom /*=true*/, bool needAddNamespace /*=true*/ )
  {
    ...
    send( iq, needAddFrom, needAddNamespace );
  }

  void ClientBase::send( const IQ& iq, bool needAddFrom /*=true*/, bool needAddNamespace /*=true*/ )
  {
  ...
  {
	 ELOG("log_network")->debug("send:\t"+xml);
  ...
  {
    char r[16+1];
    sprintf( r, "%08x%08x", m_uniqueBaseId, m_nextId.increment() );
    std::string ret( r, 16 );
    return ret;
  }
  ...
  void ClientBase::disposeMessageSession( MessageSession* session )
  {
    ...
    {
	  Message msg(NULL);
	  msg.m_subtype = Message::Invalid;
	  msg.m_thread = "disposeMessageSession";
  ...
  {	
	...
	//setCompressionImpl(new CompressionDefault(this));
  }
...
//     delete tag;
  ...
  void ClientBase::notifyMessageHandlers( Tag* tag )
  {
	  boost::shared_ptr<Tag> shared_tag(tag->clone());

	  if (!process_msg_)
	  {
		  donotifyMessageHandlers(shared_tag);
	  }
	  else
	  {
		  process_msg_->connect(process_msg_->num_slots(), boost::bind(&ClientBase::donotifyMessageHandlers, this, shared_tag));
	  }
  }

  void ClientBase::donotifyMessageHandlers( boost::shared_ptr<Tag> shared_tag )
  {

	 Message msg( shared_tag.get() );
	 m_seFactory->addExtensions( msg, shared_tag.get() );
...
// FIXME don't use '== 0' here
	...
	MessageSessionList::const_iterator it_session = m_messageSessions.begin();
	for( ; it_session != m_messageSessions.end(); ++it_session )
	{
		if( msg.from().full().find(DISCUSSIONS_DOMAIN) != std::string::npos &&
			(*it_session)->target().full().find(DISCUSSIONS_DOMAIN) != std::string::npos)
		{
			(*it_session)->handleMessage( msg );
			return;
		}
		
		if( msg.from().full().find(GROUPS_DOMAIN) != std::string::npos &&
			(*it_session)->target().full().find(GROUPS_DOMAIN) != std::string::npos)
		{
			(*it_session)->handleMessage( msg );
			return;
		}
		...
		if( msg.findExtension(kExtUser_msg_filter_filedak_message) &&
			(*it_session)->target().full().find(FILEDAK_DOMAIN) != std::string::npos)
		{
			(*it_session)->handleMessage( msg );
			return;
		}

		if( msg.from().full().find(LIGHTAPP_DOMAIN) != std::string::npos &&
			(*it_session)->target().full().find(LIGHTAPP_DOMAIN) != std::string::npos)
		{
			(*it_session)->handleMessage( msg );
			return;
		}

		if( msg.from().full().find(GROWTH_DOMAIN) != std::string::npos &&
			(*it_session)->target().full().find(GROWTH_DOMAIN) != std::string::npos)
		{
			(*it_session)->handleMessage( msg );
			return;
		}
    ...
    if( msHandler )
    {
      if( msg.subtype() == Message::Chat && msg.body().empty() && msg.html().empty() )
...
//       util::ForEach( m_messageHandlers, &MessageHandler::handleMessage, msg ); // FIXME remove for 1.1
  ...
  tm ClientBase::get_servertime() const
  {
	  return m_server_base_time.m_time;
  }

  long long ClientBase::get_timetick() const
  {
	  return m_server_base_time.m_tick;
  }

  void ClientBase::cleanup()
  {
	  if(m_parser)m_parser->cleanup(true);
  }
