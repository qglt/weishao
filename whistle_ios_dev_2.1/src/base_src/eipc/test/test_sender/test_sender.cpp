...
using namespace std;
#define DEST_IPC_TARGET "test_recv"
#define MY_IPC_NAME "test_sender"
...
void get_net_card()
{
	...
	eipc_rpc::instance().async_call(DEST_IPC_TARGET,jobj,bind2f(&get_net_card_cb,_1));
}
void get_services_list()
{
	...
	eipc_rpc::instance().async_call(DEST_IPC_TARGET,jobj,bind2f(&get_services_list_cb,_1));
}
...
void connect_service()
{
	...
	eipc_rpc::instance().async_call(DEST_IPC_TARGET,jobj,bind2f(&connect_service_cb,_1));
}
void disconnect()
{
	json::jobject jobj;
	jobj["method"] = "disconnect_network";
	eipc_rpc::instance().async_call(DEST_IPC_TARGET,jobj);
}
void test_callback(json::jobject jobj)
{
	print_log(jobj.to_string());
}
void do_test()
{
	json::jobject jobj;
	jobj["method"] = "do_test";
	eipc_rpc::instance().async_call(DEST_IPC_TARGET,jobj, boost::bind(&test_callback, _1));
}
